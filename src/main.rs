...
use dotenv;
use ethers::{
    etherscan::{blocks::BlockNumberByTimestamp, Client},
    providers::{Http, Middleware, Provider},
    types::{BlockNumber, Chain, TransactionReceipt},
};

async fn get_gas_price_at_block(block_number: BlockNumber) -> Result<u64, Error> {
    let provider: Provider<Http> = ethers::providers::Provider::<Http>::try_from(String::from(
        dotenv::var("ARB_RPC_URL").unwrap(),
    ))
    .unwrap();
    let transactions: Vec<TransactionReceipt> =
...
async fn fetch_gas_prices(
    ...
    for i in 0..intervals {
        ...
        let block_number: BlockNumberByTimestamp =
            match client.get_block_by_timestamp(timestamp, "after").await {
                Ok(bn) => bn,
                Err(e) => {
                    eprintln!("{:?}", e);
                    exit(0);
        ...
        let block_number: BlockNumber = block_number.block_number;
